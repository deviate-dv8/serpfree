...
export default class SERPScraper extends PreprocessService {
  ...
  private googleContextResetInProgress: boolean = false;
  private cleanUpIntervalId: NodeJS.Timeout | null = null;
  private pendingGoogleTasks: SearchTask[] = [];

  constructor(maxTabs: number = 1000, maxQueueSize: number = 1000) {
    super();
    this.maxTabs = Math.max(1, maxTabs); // Ensure at least 1 tab
  ...
  private async resetGoogleContext() {
    if (this.googleContextResetInProgress) return;
    this.googleContextResetInProgress = true;
    ...
    try {
      ...
      if (this.googleContext) await this.googleContext.close();
      if (this.browser) {
        this.googleContext = await this.browser.createBrowserContext();
      }
      ...
      // Retry pending Google tasks
      const pendingTasks = [...this.pendingGoogleTasks];
      ...
      this.googleContextResetInProgress = false;
  ...
  private async getAvailableTab(searchEngine: SearchEngine): Promise<TabPool> {
    ...
    const currentTabs = this.tabPool.filter(
      (tab) => tab.contextType === contextType,
    ).length;
    const maxTabsForContext = useGoogleContext
      ? Math.max(1, Math.floor(this.maxTabs / 2)) // Reserve at least 1 for each context
      : Math.max(1, this.maxTabs - 1); // But ensure at least 1 tab for default

    if (currentTabs >= maxTabsForContext) {
  ...
  private startIdleTabCleanup() {
    this.cleanUpIntervalId = setInterval(async () => {
      ...
      // Keep at least 1 tab per context
  ...
  private async executeSearch(tab: TabPool, task: SearchTask) {
    ...
    try {
      ...
      if (captchaDetected && task.searchEngine === SearchEngine.GOOGLE) {
        console.log(`Captcha detected in task ${task.id}`);
      ...
      if (captchaDetected && task.searchEngine === SearchEngine.GOOGLE) {
        // Store the task for retry after context reset
        if (task.retries < MAX_RETRIES) {
          task.retries++;
          this.pendingGoogleTasks.push(task);
          console.log(
            `Queued task ${task.id} for retry (attempt ${task.retries})`,
          );
        } else {
          task.reject(new Error("Max retries reached for Google search"));
        }
        // Initiate context reset if not already in progress
        if (!this.googleContextResetInProgress) {
